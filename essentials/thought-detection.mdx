...
This client script includes a simple `ENABLE_THOUGHT_DETECTION` flag. When set to `true`, it automatically adjusts the WebSocket URL and the VAD settings, then listens for the specific `complete_thought` message from the server.
    ...
    <CodeGroup dropdown>
        ...
        WEBSOCKET_URL = BASE_URL + ("?detect_thoughts=true" if ENABLE_THOUGHT_DETECTION else "")
        ...
        if (!API_KEY || API_KEY === "YOUR_API_KEY") {
          console.warn("⚠️  Using placeholder API key. Set FENNEC_API_KEY for real requests.");
        }
        ...
        (async () => {
          ...
          ws.on("open", async () => {
            try {
              ...
              console.log("\n🎙️  Streaming mic… speak a full thought and pause. (Ctrl+C to stop)");
              ...
              try {
                ...
                console.error("\n❌ Failed to access microphone. Ensure an input device is available.");
                shutdown(micInstance);
              }
            } catch (e: any) {
