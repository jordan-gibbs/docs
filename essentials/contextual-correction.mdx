...
### 1. Define your context and enable the flag

In your Python script, create a string containing your contextual hints and set a boolean flag to `True`.

```python
# Provide hints to the AI to recognize proprietary or uncommon words, all in natural language.
# You can also use this to change some simpler forms of formatting as well, such as capitalization or depluralization.
...
# A flag to enable the AI correction pass.
apply_correction_flag = True
```

### Add the parameters to your request
Include context and apply_contextual_correction in the data part of your requests.post call.
...
POLL_INTERVAL_S = 3
=== NEW: Contextual Correction Configuration ===
1. The context to send for the AI correction pass.
context_string = "The company they are talking about is called Bartholomew Bros, correct any potential misinterpretations in the script."
2. A flag to enable the AI correction pass.
apply_correction_flag = True
def transcribe_with_context():
"""Submits an audio file with contextual correction enabled and polls for the result."""
...
<AccordionGroup>
  <Accordion icon="link" title="URL Example">
    ...
    # url_transcription_with_context.py
import time
import json
import requests
...
def submit_job():
    """Submit a URL transcription job with contextual correction; return job_id."""
...
The Fennec fox mainly eats insects, small mammals, and birds. It has a lifespan of up to fourteen years in captivity and about ten years in the wild. Pups are preyed upon by the pharaoh eagle owl. Both adults and pups may possibly fall prey to jackals and striped hyenas. Fennec families dig out burrows in the sand for habitation and protection, which can be as large as 120 meters squared, and adjoin the burrows of other families. Precise population figures are not known, but are estimated from the frequency of sightings. These indicate that the fennec fox is currently not threatened by extinction. Knowledge of social interactions is limited to information gathered from captive animals. The fennec fox is commonly trapped for exhibition or sale in North Africa, and it is considered an exotic pet in some parts of the world.
    ```
  </Card>
